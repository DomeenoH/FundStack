...
export default function DonationForm({ config, onPaymentMethodChange, onSubmitSuccess }: DonationFormProps) {
  ...
  const [pendingData, setPendingData] = useState<DonationFormData | null>(null);
  ...
  const onCaptchaVerify = async (isValid: boolean) => {
    if (isValid && pendingData) {
      setShowCaptcha(false);
      await processDonation(pendingData);
      setPendingData(null);
    }
  };

  const processDonation = async (data: DonationFormData) => {
    setSubmitError(null);
    setSuccess(false);
